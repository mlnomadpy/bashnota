<template>
  <!--
    Ported node-view component (Phase 0). Identical to YoutubeBlock.vue except the
    root is a plain <div> instead of TipTap's <node-view-wrapper>: the wrapper
    element is now owned by our in-house VueNodeView bridge, which renders this
    component INTO its own DOM node. Behaviour is unchanged.
  -->
...
// Props supplied by the in-house VueNodeView bridge. Same shape TipTap gave the
// original component, so the logic below is unchanged.
interface Props {
  node: {
    attrs: NodeAttrs
  }
