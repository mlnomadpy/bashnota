...
export const useEditorAIActionsStore = defineStore('editorAiActions', () => {
  ...
  const codeActions = computed(() =>
    enabledCustomActions.value.filter(
      (action) =>
  ...
  const builtInActions: CustomAIAction[] = [
    ...
    },
    {
      ...
      icon: 'Zap',
      prompt:
        'Please optimize this {{language}} code for better performance:\n\n```{{language}}\n{{code}}\n```\n\nProvide:\n1. Optimized code\n2. Explanation of optimizations\n3. Expected performance improvement',
      category: 'transformation',
      isBuiltIn: true,
      isEnabled: true,
      outputType: 'code',
    ...
    },
    {
      ...
      icon: 'FileText',
      prompt:
        'Please add comprehensive documentation to this {{language}} code:\n\n```{{language}}\n{{code}}\n```\n\nInclude:\n1. Function/class documentation\n2. Inline comments for complex logic\n3. Parameter descriptions\n4. Return value descriptions',
      category: 'transformation',
      isBuiltIn: true,
      isEnabled: true,
      outputType: 'code',
  ...
  const saveSettings = () => {
    try {
      localStorage.setItem(
        'ai-code-preferences',
        JSON.stringify(withoutApiKeys(state.providerSettings)),
      )
  ...
  const executeCustomAction = async (
    ...
    if (provider === 'gemini' && !apiKey) {
      throw new Error(
        `API key is required for ${provider} provider. Please configure your API key in Settings > AI Assistant > AI Providers.`,
      )
    }
    ...
    if (provider === 'webllm') {
      ...
      if (!webllmAutoLoad) {
        throw new Error(
          `WebLLM auto-loading is disabled. Please enable auto-loading or manually load a model in Settings > AI Assistant > AI Providers.`,
        )
      }
    }
...
- Has Output: ${context.hasOutput ? 'Yes' : 'No'}`
      ...
      if (
        (action.id === 'fix-error' || action.category === 'debugging') &&
        (executionMetadata || outputContext)
      ) {
      ...
      if (provider === 'webllm' && errorMessage.includes('not initialized')) {
        ...
        throw new Error(
          `WebLLM model not initialized. Please go to Settings > AI Assistant > AI Providers and load a WebLLM model, or select a different AI provider.`,
        )
      }
