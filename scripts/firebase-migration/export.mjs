...
export function normalizeAuthExport(authExport) {
  ...
  if (!Array.isArray(users)) throw new Error('Firebase Auth export must contain a users array')
...
export function assembleExport({ watermark, authExport, collections, storageManifest = [] }) {
  ...
  for (const name of EXPORT_COLLECTIONS) {
    ...
    firestore[name] = [...collections[name]].sort((a, b) => String(a.id ?? `${a.notaId}/${a.userId}`).localeCompare(String(b.id ?? `${b.notaId}/${b.userId}`))).map(stableValue)
  }
  return stableValue({ version: 1, watermark: new Date(watermark).toISOString(), authUsers: normalizeAuthExport(authExport), firestore, storageManifest })
}
