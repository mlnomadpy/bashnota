...
export class FileTarget {
  ...
  async startRun({ runId, manifest, dryRun }) {
    ...
    if (prior && prior.manifestHash !== manifest.manifestHash) throw Object.assign(new Error('run manifest conflict'), { code: '23505' })
    this.runs.set(runId, { manifestHash: manifest.manifestHash, dryRun, state: 'running' })
  }
  ...
  async apply(record) {
    ...
    this.records.set(record.keyHash, structuredClone(record))
  }
  async complete(record) { this.journal.get(record.keyHash).state = 'applied' }
  async fail(record, errorClass) { Object.assign(this.journal.get(record.keyHash), { state: 'failed', errorClass }) }
  ...
  async rollback(runId) {
    for (const [key, journal] of this.journal) if (journal.runId === runId) { this.records.delete(key); journal.state = 'rolled-back' }
